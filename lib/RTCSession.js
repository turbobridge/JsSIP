...
module.exports = class RTCSession extends EventEmitter
{
      ...
      switch (request.method)
      {
        case JsSIP_C.ACK:
          ...
          if (this._late_sdp)
          {
            ...
            const e = { type: 'answer', sdp: request.body, success, failure };

            debug('emit "remotemedia"');
            this.emit('remotemedia', e);
          }
          else
          if (!this._is_confirmed)
          {
  ...
  isReadyToReOffer()
  {
    ...
    {
      debug('isReadyToReOffer() | invalid state');
  ...
  _receiveReinvite(request)
  {
    ...
    if (!request.body)
    {
      ...
      const e = { type: 'offer', sdp: null, success, failure };
    ...
    this._processInDialogSdpOffer(request, (desc) =>
    {
      if (this._status === C.STATUS_TERMINATED)
      {
        return;
  ...
  _processInDialogSdpOffer(request, onSuccess)
  {
    ...
    const e = { type: 'offer', sdp: request.body, success, failure };
  ...
  _receiveInviteResponse(response)
  {
    ...
    switch (true)
    {
      ...
      case /^1[0-9]{2}$/.test(response.status_code):
      {
        ...
        const e = { type: 'answer', sdp: response.body, success, failure };
  ...
  sendReinvite(options = {})
  {
    ...
    if (!sdpOffer)
    {
      throw new TypeError('sdpOffer required');
    }
