...
module.exports = class RTCSession extends EventEmitter
{
  ...
  connect(target, options = {}, initCallback)
  {
    ...
    this._rtcConstraints = rtcConstraints;
    ...
    this._sendInitialRequest(mediaConstraints, rtcOfferConstraints);
  }
  ...
  _close(terminateSession = true)
  {
    debug('close()');

    if (this._status === C.STATUS_TERMINATED)
    {
      debug('close() | STATUS_TERMINATED, returning early');
      return;
    }
    ...
    if (this._connection)
    {
      try
      {
        debug('close() | calling RTCPeerConnection.close()');
  ...
  {
    this._connection = new RTCPeerConnection(this._pcConfig, this._rtcConstraints);
  ...
  _receiveReinvite(request)
  {
    ...
    if (data.createNewPeerConnection) {
      debug('receiveReinvite() | creating new peerconnection');

      this._connectionPromiseQueue = this._connectionPromiseQueue
        .then(() => this._close(false))
        .then(() => this._createRTCConnection())
        .then(() => {
   ...
   */
  _sendInitialRequest(mediaConstraints, rtcOfferConstraints)
  {
